...
def user_objects_get(slug):
    query_args = request.args
    resp, code = api.methods.user_objects_get(slug, query_args)
    return jsonify(resp), code
...
def thread_create_view(slug):
    payload = request.get_json(silent=True)
    resp, code = api.methods.thread_create(slug, payload)
    return jsonify(resp), code


@app.route(api.get_url('/forum/<slug>/threads'), methods=['GET'])
def thread_objects_get_view(slug):
    query_args = request.args
    resp, code = api.methods.thread_objects_get(slug, query_args)
    return jsonify(resp), code
...
def post_create_view(slug_or_id):
    payload = request.get_json(silent=True)
    resp, code = api.methods.post_create(slug_or_id, payload)
    return jsonify(resp), code


@app.route(api.get_url('/thread/<slug_or_id>/posts'), methods=['GET'])
def post_objects_get_view(slug_or_id):
    query_args = request.args
    resp, code = api.methods.post_objects_get(slug_or_id, query_args)
    return jsonify(resp), code
